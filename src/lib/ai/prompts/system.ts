...
1. **Use theme.json for all design tokens.** Colors should reference palette slugs via "var:preset|color|slug" format. Font families via "var:preset|font-family|slug".
2. **Create visually impressive, non-generic themes.** Use interesting color palettes, thoughtful typography, and sophisticated layouts.
3. **Use cover blocks for hero sections** with background images from Unsplash (use realistic placeholder URLs like https://images.unsplash.com/photo-1234567890).
4. **Every template must start with a header template-part and end with a footer template-part.**
5. **Use clamp() for responsive font sizes** in theme.json styles.
6. **Create at least 2-3 patterns** that showcase the theme's design (hero, features, CTA).
7. **The index template must include a query loop** for displaying posts.
8. **Use core/group with layout type "constrained" for content width containers.**
